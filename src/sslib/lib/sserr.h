...
#define SS_RETURNS_H5E_error_tP         const H5E_error_t *_retval = NULL
...
typedef struct ss_err_info_t {
    ...
    const H5E_error_t   *info;                  /* information about the error, points directly into error stack */
} ss_err_info_t;
...
#define SS_ENTER(_function_name_, _return_type_) {                                                                              \
    ...
    if (0==sslib_g.call_depth++) {                                                                                              \
        H5Eget_auto_stack(H5E_DEFAULT, &_efunc,&_edata);                                                                        \
    ...
    if (!ss_if_inited_g && ss_if_init()<0) {                                                                                    \
        ...
        H5Epush_stack(_estack, __FILE__, _func, __LINE__, ss_eclass_g, ss_if_major_g, SS_MINOR_INIT, "");                       \
...
#define SS_ERROR_FMT(_ecode_, _fmt_args_) do {                                                                                 \
    ...
    H5Epush_stack(_estack, __FILE__, _func, __LINE__, ss_eclass_g, ss_if_major_g, SS_MINOR_##_ecode_,                          \
...
#define SS_LEAVE(_retval_)                                                                                                     \
    ...
    if (_efunc) H5Eset_auto_stack(H5E_DEFAULT, _efunc,_edata);                                                                 \
                                                                                                                               \
    if (_doing_cleanup) {                                                                                                      \
...
const H5E_error_t *ss_err_info(hid_t estack, int position);
herr_t ss_err_info_cb(unsigned n, const H5E_error_t *info, void *_walker);
herr_t ss_err_print(hid_t estack, void *econtrol);
herr_t ss_err_print_cb(unsigned n, const H5E_error_t *einfo, void *econtrol);
