               ...
               )
{
    ...
    if (blob && SS_BLOB(blob)->dsetaddr) {
        ...
        SS_ASSERT(gblob->d[d_idx].stat.objno==sb.objno);
    ...
    for (d_idx=0; d_idx<gblob->d_nused; d_idx++) {
        if (gblob->d[d_idx].stat.objno==sb.objno && gblob->d[d_idx].stat.fileno==sb.fileno) {
            break;
        }
    }
    ...
    if (gblob->d[d_idx].dset <= 0) {
        ...
        if (H5Gget_objinfo(gblob->storage, dsetname, FALSE, &sb_link)>=0) {
            if (sb.objno!=sb_link.objno) SS_ERROR_FMT(CORRUPT, ("mis-linked blob storage for `%s'", dsetname));
        } else {
    ...
    if (blob) {
        SS_ASSERT(sb.objno==gblob->d[d_idx].stat.objno);
        SS_BLOB(blob)->dsetaddr = sb.objno;
                ...
                )
{
    ...
    SS_ASSERT(gblob);
    SS_ASSERT(SS_BLOB(blob)->dsetaddr==gblob->d[d_idx].stat.objno);
                    ...
                    )
{
    ...
    for (a_idx=0, ndsets[self]=0; a_idx<gblob->a_nused; a_idx++) {
        if (one_dset_addr>0) {
            if (gblob->d[gblob->a[a_idx].d_idx].stat.objno==one_dset_addr) {
                a_start = a_idx;
                ndsets[self] = 1;
                break;
            }
            ...
            ndsets[self]++;
        } else if (gblob->d[gblob->a[a_idx].d_idx].stat.objno!=gblob->d[gblob->a[a_idx-1].d_idx].stat.objno) {
            ndsets[self]++;
        }
    }
    ...
    while (ndsets_total) {
        ...
        if (a_start<gblob->a_nused) {
            ...
            for (a_idx=1; a_start+a_idx<gblob->a_nused; a_idx++) {
                if (gblob->d[gblob->a[a_start+a_idx].d_idx].stat.objno!=dset_info[self].objno) break;
            }
         ...
         * array. */
        for (d_idx=0; d_idx<gblob->d_nused; d_idx++) if (gblob->d[d_idx].stat.objno==smallest_objno) break;
...
ss_blob_boot_cb(hid_t group, const char *name, void *_gblob)
{
    ...
    if (objno!=gblob->d[n].stat.objno) SS_ERROR_FMT(CORRUPT, ("blob dataset `%s' name is not ohdr address", name));
        ...
        if (completed) {
            ...
            if (gblob->agg.asynchdf5) {
                ...
                if (!H5Pexist(dxpl, "async")) {
                    ...
                    if (H5Pinsert(dxpl, "async", sizeof(void*), &async_func, NULL, NULL, NULL, NULL, NULL, NULL)<0)
                        SS_ERROR(HDF5);
                    if (H5Pinsert(dxpl, "async_req", sizeof(aiocb), &aiocb, NULL, NULL, NULL, NULL, NULL, NULL)<0)
                        SS_ERROR(HDF5);
...
ss_blob_didx(ss_blob_t *blob)
{
    ...
    if (0==SS_BLOB(blob)->m.d_idx) {
        for (d_idx=0; d_idx<gblob->d_nused; d_idx++) {
            if (gblob->d[d_idx].stat.objno==SS_BLOB(blob)->dsetaddr) {
                SS_BLOB(blob)->m.d_idx = d_idx;
                break;
            }
        }
