...
{
    SS_ENTER(ss_err_info, H5E_error_tP);
    int         stksize;
    ...
    if (H5Ewalk_stack(estack, H5E_WALK_UPWARD, ss_err_info_cb, &walker)<0) SS_ERROR(HDF5);
 ...
 */
herr_t
ss_err_info_cb(unsigned n, const H5E_error_t *info, void *_walker)
{
...
ss_err_print(hid_t estack, void *_econtrol)
{
    ...
    if (estack==H5E_DEFAULT) {
        ...
        if (econtrol->fd>=0) {
            ...
            if (H5Ewalk_stack(estack, H5E_WALK_DOWNWARD, ss_err_print_cb, _econtrol)<0) SS_ERROR(HDF5);
 ...
 */
herr_t
ss_err_print_cb(unsigned n, const H5E_error_t *einfo, void *_econtrol)
{
